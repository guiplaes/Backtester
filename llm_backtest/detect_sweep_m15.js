// Sweep on M15 + relaxed wick threshold
const fs = require('fs');
...
const WICK_MIN_ATR = 0.2;  // slightly relaxed for M15
const candidates = [];
...
function simulate(c) {
  ...
  for (let i = c.idx + 1; i <= c.idx + 12 && i < bars.length; i++) {
    const b = bars[i];
    if (c.side === 'LONG') { if (b.low <= sl) return { outcome:'SL', pnlR:-1, sl, tp }; if (b.high >= tp) return { outcome:'TP', pnlR:2, sl, tp }; }
    else { if (b.high >= sl) return { outcome:'SL', pnlR:-1, sl, tp }; if (b.low <= tp) return { outcome:'TP', pnlR:2, sl, tp }; }
  }
...
console.log(`\n=== Liquidity Sweep on M15 (${bars.length} bars, 1 week) ===`);
...
if (n > 0) {
  ...
  results.forEach((r,i) => console.log(`${String(i).padStart(2)}: ${r.timeStr} ${r.side.padEnd(5)} @${r.entry.toFixed(2)} swept ${r.swingLevel.toFixed(2)} (wick ${(r.wickSize/r.atr14).toFixed(2)}xATR) → ${r.outcome.padEnd(8)} (${r.pnlR.toFixed(2)}R)`));
}

fs.writeFileSync(path.join(__dirname, 'sweep_m15_results.json'), JSON.stringify({ bars, atr14, rsi14, results }, null, 2));
