// Same sweep filter on H1 data (5 weeks)
const fs = require('fs');
...
for (let i = SWING_LOOKBACK; i < bars.length - 12; i++) {
  ...
  const shortSetup = sweptHigh && closedBelow && wickAbove > a * WICK_MIN_ATR && b.close < b.open;

  const sweptLow = b.low < swingLow;
  const closedAbove = b.close > swingLow;
  const wickBelow = swingLow - b.low;
  const longSetup = sweptLow && closedAbove && wickBelow > a * WICK_MIN_ATR && b.close > b.open;
...
function simulate(c) {
  ...
  let sl, tp;
  if (c.side === 'LONG') { sl = c.swingLevel - atr * 0.1; tp = entry + (entry - sl) * 2.0; }
  else { sl = c.swingLevel + atr * 0.1; tp = entry - (sl - entry) * 2.0; }
  const r_dist = Math.abs(entry - sl);
  for (let i = c.idx + 1; i <= c.idx + 12 && i < bars.length; i++) {
    const b = bars[i];
    if (c.side === 'LONG') {
      if (b.low <= sl) return { outcome:'SL', pnlR:-1, sl, tp };
      if (b.high >= tp) return { outcome:'TP', pnlR:2, sl, tp };
    } else {
      if (b.high >= sl) return { outcome:'SL', pnlR:-1, sl, tp };
      if (b.low <= tp) return { outcome:'TP', pnlR:2, sl, tp };
    }
  }
  const last = bars[Math.min(c.idx + 12, bars.length - 1)];
  const mv = c.side === 'LONG' ? (last.close - entry) : (entry - last.close);
  return { outcome:'TIMEOUT', pnlR: mv / r_dist, sl, tp };
}
...
console.log(`\n=== Liquidity Sweep on H1 (${bars.length} bars, ~5 weeks) ===`);
...
if (n > 0) {
  ...
  results.forEach((r,i) => console.log(`${String(i).padStart(2)}: ${r.timeStr} ${r.side.padEnd(5)} @${r.entry.toFixed(2)} swept ${r.swingLevel.toFixed(2)} → ${r.outcome.padEnd(8)} (${r.pnlR.toFixed(2)}R)`));
}

fs.writeFileSync(path.join(__dirname, 'sweep_results_h1.json'), JSON.stringify({ bars, atr14, rsi14, results }, null, 2));
