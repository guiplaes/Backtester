// Build REJECT/BREAK contexts for level touches
const fs = require('fs');
const path = require('path');

const data = JSON.parse(fs.readFileSync(path.join(__dirname, 'level_touches.json'), 'utf8'));
...
// Approach character: how did price get to this level?
function approachChar(barsArr, idx, levelPrice) {
  ...
  let momentum = Math.abs(netMove) > 0 ? Math.abs(netMove) : 0.01;
  return { netMove, momentum, volTrend, avgVol };
}

// Prior tests of this level (within today's data)
function priorTests(barsArr, idx, levelPrice, atr) {
  ...
  for (let j = Math.max(0, idx - 40); j < idx; j++) {
    ...
    if (Math.abs(barsArr[j].high - levelPrice) < proxRange || Math.abs(barsArr[j].low - levelPrice) < proxRange) {
      tests++;
    }
  }
  return tests;
...
const ctxDir = path.join(__dirname, 'contexts_levels');
...
touches.forEach((t, idx) => {
  ...
  const lvl = t.level;
  const a = t.atr14;
  const ap = approachChar(bars, i, lvl.price);
...
`;
  ...
  fs.writeFileSync(path.join(ctxDir, `L_${String(idx).padStart(3,'0')}.txt`), ctx);
});

fs.writeFileSync(path.join(__dirname, 'level_meta.json'), JSON.stringify({ touches, bars }));
console.log(`Built ${touches.length} level-touch contexts in contexts_levels/`);
