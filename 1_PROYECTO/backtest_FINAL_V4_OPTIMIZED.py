"""
PORTFOLIO V4 OPTIMIZED — Configs MILLORS trobats al stable_opt
================================================================
Substitueix les configs antigues per les noves òptimes 6/6 anys.
"""
import pandas as pd
import numpy as np
import sys
...
# V4 — TOP optimal configs (basat en stable_opt deep findings)
STRATEGIES = [
    # EURGBP × multiple TFs BOTH
    ...
    # GBPCHF SHORT — múltiple TFs (TOTS 6/6 anys)
    {'name':'GBPCHF D1 S','pair':'GBPCHF','tf':'1D','ma':50,'levels':[-0.5,-1.0,-1.5,-2.0],'stop':-4.0,'dir':'short'},
    {'name':'GBPCHF H4 S150','pair':'GBPCHF','tf':'4h','ma':150,'levels':[-0.5,-1.0,-1.5,-2.0],'stop':-4.0,'dir':'short'},
    {'name':'GBPCHF H4 S300','pair':'GBPCHF','tf':'4h','ma':300,'levels':[-0.5,-1.0,-1.5,-2.0],'stop':-4.0,'dir':'short'},
    ...
    # USDCAD H1 BOTH dirs (S 6/6, L variable)
...
print(f"V4 OPTIMIZED ({len(STRATEGIES)} estratègies)...", flush=True)
...
print()
print("="*120)
print(f"V4 OPTIMIZED — {len(STRATEGIES)} estratègies")
print("="*120)
...
# TG
msg = f"🏆 <b>V4 OPTIMIZED — {len(STRATEGIES)} estrategies</b>%0A%0A"
for r in results:
