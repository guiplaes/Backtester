"""
V6 HYBRID sobre 10 ANYS (2015-2026)
====================================
Inclou: SNB Jan-2015, Brexit 2016, Trump elections, COVID 2020, BoJ pivots.
Test definitiu del sistema sobre escenaris reals de crisi.
"""
import pandas as pd
import numpy as np
import sys
...
print(f"V6 HYBRID 10-YEAR test ({len(STRATEGIES)} strats)...", flush=True)
...
def run_simulation(lot):
    ...
    return {'lot':lot,'annual':annual,'max_dd_pct':dd_pct.max(),
            ...
            'max_pos':int(up.sum(axis=1).max()),'tr':tr,'eq':eq,
            'span_y':span_y,'final':fin}
...
print("\n" + "="*120)
print(f"V6 HYBRID 10 ANYS ({results[0]['span_y']:.1f}y reals)")
print("="*120)
print(f"  {'Lot':<6} {'Annual':>9} {'DD%':>7} {'Loss init%':>11} {'Calmar':>7} {'Total Net':>12}")
for r in results:
    print(f"  {r['lot']:<6} +{r['annual']:>5.2f}% {r['max_dd_pct']:>5.2f}% {r['loss_init_pct']:>9.2f}% {r['calmar']:>6.2f} ${r['final']:>+10,.0f}")
...
for yr in yd.index:
    ...
    print(f"  {yr}: ${p:>+10,.0f} ({sgn}{abs(pct):.2f}%)")
print(f"\nAnys positius: {yrs_pos}, negatius: {yrs_neg}")

# Specific events check
print("\n" + "="*120)
print("ANALISI EVENTS CRITICS (DD per dia):")
print("="*120)
daily = best['eq'].resample('D').last().dropna()
daily_change = daily.pct_change() * 100

# Check top 10 worst days
worst_days = daily_change.nsmallest(10)
print("\nTop 10 dies pitjors:")
for d, v in worst_days.items():
    eq_at = daily.loc[d]
    print(f"  {d.date()}: {v:+.2f}% (Equity ${eq_at:,.0f})")

# Specific dates
critical_dates = [
    ('2015-01-15', 'SNB break (CHF +20%)'),
    ('2016-06-24', 'Brexit (GBP -10%)'),
    ('2020-03-15', 'COVID crash'),
    ('2022-09-26', 'BoE crisis'),
    ('2024-09-25', 'BoJ surprise hike'),
]
print("\nEvents critics:")
for date_str, desc in critical_dates:
    target = pd.Timestamp(date_str).date()
    if target in [d.date() for d in daily.index]:
        idx = [i for i, d in enumerate(daily.index) if d.date()==target][0]
        eq_at = daily.iloc[idx]
        if idx > 0:
            change = (daily.iloc[idx] - daily.iloc[idx-1]) / daily.iloc[idx-1] * 100
            print(f"  {date_str} ({desc}): equity ${eq_at:,.0f}, change {change:+.2f}%")
        else:
            print(f"  {date_str} ({desc}): equity ${eq_at:,.0f}")
    else:
        print(f"  {date_str} ({desc}): no data aquell dia")

# TG
msg = f"📊 <b>V6 HYBRID 10 ANYS</b> (2015-2026)%0A%0A"
for r in results:
    msg += f"Lot {r['lot']}: <b>+{r['annual']:.1f}%</b>/any DD <b>{r['max_dd_pct']:.1f}%</b> Loss inicial {r['loss_init_pct']:.1f}%%0A"
msg += f"%0AAnys positius: {yrs_pos}/{yrs_pos+yrs_neg}"
tg_send(msg)
print("\nDONE")
