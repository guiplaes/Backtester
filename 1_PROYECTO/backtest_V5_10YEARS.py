"""V5 BALANCED sobre 10 ANYS — max diversificacio (15 pairs)."""
import pandas as pd
import numpy as np
import sys
sys.path.insert(0, '.')
...
# V5 needs MORE pairs. Need to download these too. For now use available.
PAIRS_INFO = {
    ...
    # Extra pairs V5 needs (might not all have 10y):
    'EURAUD':{'cost':0.40,'pip':1000},'EURCAD':{'cost':0.40,'pip':1000},
    'AUDUSD':{'cost':0.30,'pip':1000},'NZDUSD':{'cost':0.40,'pip':1000},
...
# V5 BALANCED 26 strats. Skip strategies that lack 10y data.
STRATEGIES_RAW = [
    ...
    # V5 extras (may need to skip if 10y data not available):
...
# Filter only strategies with 10y data available
import os
STRATEGIES = []
for s in STRATEGIES_RAW:
    if os.path.exists(PAIR_FILES[s['pair']]):
        STRATEGIES.append(s)
    else:
        # Fallback to 5y file if exists
        fallback = f"{s['pair'].lower()}_dk_m5_5y.csv"
        if os.path.exists(fallback):
            PAIR_FILES[s['pair']] = fallback
            STRATEGIES.append(s)
        else:
            print(f"Skip {s['name']} - no data")

print(f"V5 BALANCED 10y test ({len(STRATEGIES)} strats)...", flush=True)
...
print(f"\nV5 10 ANYS ({results[0]['span_y']:.1f}y)")
for r in results:
...
msg = f"📊 <b>V5 BALANCED 10 ANYS</b>%0A%0A"
for r in results:
    msg += f"Lot {r['lot']}: +{r['annual']:.1f}%/any DD {r['max_dd_pct']:.1f}%%0A"
