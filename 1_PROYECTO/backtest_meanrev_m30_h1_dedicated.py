"""
M30 + H1 dedicated sweep — params adaptats a cada TF
=====================================================
NO assumim res de H4. Sweep complet per cada TF independent.
Multi-period SMA, multi-level, multi-stop combinations.
"""
import pandas as pd
import numpy as np
import time
...
# Periods adaptats per cada TF (SMA period en bars):
# M30: 50bars=25h, 100bars=50h, 200bars=100h=4d, 400bars=200h=8d, 800bars=16d, 1600=32d
# H1: 50bars=2d, 100bars=4d, 200bars=8d, 400bars=16d, 800bars=32d, 1600=64d
TF_MA = {
    'M30': [50, 100, 150, 200, 300, 500, 800, 1200, 1600],
    'H1':  [50, 100, 150, 200, 300, 500, 800, 1200, 1600],
}

# Levels — per M30/H1 amb més soroll, levels més espaiats
LEVELS_GRID = [
    [-1.0],[-1.5],[-2.0],
    [-1.0,-2.0],[-1.5,-2.5],[-2.0,-3.0],[-2.5,-3.5],
    [-1.0,-2.0,-3.0],[-1.5,-2.5,-3.5],[-2.0,-3.0,-4.0],
    [-0.5,-1.5,-2.5],[-1.0,-1.5,-2.0,-2.5],[-1.0,-2.0,-3.0,-4.0],
]
STOPS = [-3.0,-3.5,-4.0,-5.0,-6.0,-7.0]

print("Aggregating...", flush=True)
DATA = {
    'M30': precompute(aggregate(m5,'30min'), TF_MA['M30']),
    'H1':  precompute(aggregate(m5,'1h'), TF_MA['H1']),
}
for tf in ['M30','H1']:
    print(f"  {tf}: {DATA[tf]['n']} bars", flush=True)

print("\nDedicated M30+H1 sweep...", flush=True)
ALL = []
t0 = time.time()
for tf in ['M30','H1']:
    arrs = DATA[tf]
...
for tf in ['M30','H1']:
    print()
    print(f"TOP {tf} amb VOLUM (n>=100):")
...
# Yearly per top 5 de cada TF
for tf in ['M30','H1']:
    print()
    print("="*150)
    print(f"ANY-PER-ANY top 5 {tf}:")
    print("="*150)
    sub = [r for r in ALL if r['tf']==tf]
    for r in sub[:5]:
        s = r['stats']
        lvl = '/'.join(f'{l:.1f}' for l in r['levels'])
        print(f"\n{tf} SMA{r['ma']} {r['direction']} levels={lvl} stop={r['stop']} | TOTAL {fmt(s)}")
